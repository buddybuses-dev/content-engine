// The queue is the database. Every content item is one JSON file under
// queue/<channel>/<stage>/, and moving between stages is a file move that git records.
// That gives the pipeline a full audit trail for free, survives runner restarts, and
// needs no external service — which is what makes a 24/7 GitHub Actions pipeline cheap
// to operate.
//
// The queue is partitioned by channel before it is partitioned by stage. That ordering
// matters: it means one channel's backlog can never be confused with another's, a
// channel can be paused by simply not iterating it, and `git log queue/ai-benefits/`
// tells the whole story of one channel without noise from the others.
...
function stageDir(channel, stage) {
  const dir = STAGES[stage];
  if (!dir) throw new Error(`Unknown stage "${stage}". Valid: ${Object.keys(STAGES).join(', ')}`);
  if (!channel) throw new Error(`Missing channel for stage "${stage}" — every queue path is channel-scoped.`);
  return join(QUEUE, channel, dir);
}
...
export function createItem({ channel, title, source, brief }) {
  if (!channel) throw new Error('createItem requires a channel slug');
...
export async function listItems(channel, stage) {
  const dir = stageDir(channel, stage);
  ...
  for (const file of files) {
    ...
    try {
      ...
      // Older items predate the channel field; infer it from the path they live in
      // rather than failing, so an in-flight queue survives the upgrade.
      item.channel ??= channel;
      items.push(item);
    } catch (error) {
      throw new Error(`Corrupt queue item ${channel}/${stage}/${file}: ${error.message}`);
    }
  }
  return items;
}

export async function countItems(channel, stage) {
  const dir = stageDir(channel, stage);
...
export async function saveItem(item) {
  const dir = stageDir(item.channel, item.stage);
...
export async function advance(item, toStage, detail = '') {
  ...
  const fromPath = join(stageDir(item.channel, fromStage), `${item.id}.json`);
  item.stage = toStage;
...
export async function moveFile(item, toStage) {
  const from = join(stageDir(item.channel, item.stage), `${item.id}.json`);
  const to = join(stageDir(item.channel, toStage), `${item.id}.json`);
  await mkdir(stageDir(item.channel, toStage), { recursive: true });
  await rename(from, to);
}

/** Creates the stage directories for a channel, so a new channel needs no manual setup. */
export async function ensureChannelQueue(channel) {
  for (const stage of Object.keys(STAGES)) {
    await mkdir(stageDir(channel, stage), { recursive: true });
  }
}
